...
默认配置已改为：**按美东 09:30-10:00 开盘窗口自动执行**，对应北京时间通常为**夏令时 21:30 左右、冬令时 22:30 左右**。

修改时间：编辑 `.github/workflows/daily_analysis.yml` 中的 cron 表达式：

```yaml
schedule:
  - cron: '30 13 * * 1-5'  # 夏令时候选：北京时间 21:30
  - cron: '30 14 * * 1-5'  # 冬令时候选：北京时间 22:30
```

常用 cron 示例：
| 表达式 | 说明 |
|--------|------|
| `'30 13 * * 1-5'` + `'30 14 * * 1-5'` | 美东 09:30-10:00 开盘窗口（夏令时约北京时间 21:30，冬令时约北京时间 22:30；需配合工作流内时区判断） |
